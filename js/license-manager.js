...
const LicenseManager = (() => {
  ...
  function openUpgradePrompt() {
    ...
    modal.querySelector('.lic-upgrade-body').innerHTML = `
      ...
      <div class="lic-upgrade-actions">
        <button class="lic-btn lic-btn--primary" type="button" id="lic-upgrade-checkout">Fazer upgrade agora</button>
        <button class="lic-btn lic-btn--ghost" type="button" id="lic-upgrade-goto-plans">Ver todos os planos</button>
        <button class="lic-btn lic-btn--ghost" type="button" id="lic-upgrade-cancel">Cancelar</button>
      </div>
    `;
    document.getElementById('lic-upgrade-checkout').addEventListener('click', () => {
      modal.classList.add('hidden');
      if (typeof LicenseCheckout !== 'undefined') LicenseCheckout.open('profissional');
      else goToPlans();
    });
  ...
  function renderPlansPage(el) {
      ...
      return `
        <div class="lic-plan-card${isCurrent ? ' lic-plan-card--current' : ''}${key === 'profissional' ? ' lic-plan-card--highlight' : ''}">
          ...
          ${isCurrent
            ...
            : `<button class="lic-btn lic-btn--primary" type="button" data-checkout="${key}">Fazer upgrade</button>`
          }
        </div>
      `;
    }).join('');
    ...
    `;
    // Wires checkout buttons
    el.querySelectorAll('[data-checkout]').forEach(btn => {
      btn.addEventListener('click', () => {
        if (typeof LicenseCheckout !== 'undefined') LicenseCheckout.open(btn.dataset.checkout);
      });
    });
  }
